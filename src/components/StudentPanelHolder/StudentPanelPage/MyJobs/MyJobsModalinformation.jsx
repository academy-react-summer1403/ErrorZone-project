...
import MyJobsDetail from '../../../../core/services/api/MyJobs/MyJobsDetail'

const MyJobsModalInformation = ({ 
    courseId,
    isOpen, 
    onOpenChange, 
    jobs
    ...
    const getData=async()=>{
        ...
        const data=await MyJobsDetail(courseId)
        setdata(data)
        setLoading(false)
    }
  return (
      ...
      {Loading==false &&
        ...
        <ModalBody>
                ...
                <div className='w-full h-[350px] bg-[#E8E8E8] rounded-[16px] relative sm:w-full sm:h-[400px]'>
                    <img src={jobs.paymentInvoiceImage ? jobs.paymentInvoiceImage  : nophot} className='w-full h-full rounded-[16px]' />
                </div>
                ...
                <div className='flex flex-col gap-4 my-2'>
                    ...
                    <span className='text-[16px] text-[#272727] h-[46px] overflow-hidden dark:text-white' dir='rtl'> {jobs.aboutJob} </span>
                </div>
                <div className='flex flex-col gap-4 my-2'>
                    <h2 className='text-base text-[#787878]'> مبلغ پرداخت شده </h2>
                    <span className='text-[16px] text-[#272727] h-[46px] overflow-hidden dark:text-white' dir='rtl'>  {jobs.aboutJob} <span className="text-[13px] text-gray-500"> </span> </span>
                </div>
                <div className='flex flex-col gap-4 my-5'>
                    <h2 className='text-base text-[#787878]'> وضعیت پرداخت </h2>
                    <div className='flex justify-between w-full items-center'>
                    <span className={`${jobs.accept ? 'bg-[#17C96433] text-[#17C964]' : 'text-[#F31260] bg-[#F3126033]'} px-2 rounded-full`}> {jobs.accept ? 'تایید شده' : 'تایید نشده'} </span>
                    </div>
                </div>
                <div className='flex flex-col gap-4 my-2'>
                    <h2 className='text-base text-[#787878]'> زمان پرداخت </h2>
                    <span className='text-[16px] text-[#272727] h-[46px] overflow-hidden dark:text-white' dir='rtl'> {(jMoment(jobs.workStartDate).locale('fa').format('jD jMMMM jYYYY'))} </span>
                </div>
                <div className='flex flex-col gap-4 my-2'>
                    <h2 className='text-base text-[#787878]'> زمان پرداخت </h2>
                    <span className='text-[16px] text-[#272727] h-[46px] overflow-hidden dark:text-white' dir='rtl'> {(jMoment(jobs.workEndDate).locale('fa').format('jD jMMMM jYYYY'))} </span>
                </div>               
...
export default MyJobsModalInformation
