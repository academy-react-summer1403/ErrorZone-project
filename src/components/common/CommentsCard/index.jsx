...
const CommentsCard = ({data}) => {
    ...
    if(res.success === true) {
       return SuccessToastify(res.message)
     }else if (res.success === false) {
       return ErrorToastify("لایک با موفقیت ثبت شد")
     }
       ...
       <div className=' w-[292px] h-[42px] mx-auto grid grid-cols-2 items-center gap-[50px] '>   
         ...
         <div className=' w-[103px] h-[24px] grid grid-cols-2  mb-[-10px] pr-3 dark:text-black'> 
          <div className=' w-[49x] h-[24px] grid grid-cols-2 cursor-pointer pr-3' onClick={handleLike}> 
           {isUserLike === "0" ? (<i className='w-[24px] h-[24px] ' >  <ThumbsUpIcon  className='relative bottom-1'/>   </i>) 
          ...
          <div className=' w-[49px] h-[24px] grid grid-cols-2  mr-4 dark:text-black pr-1'> 
                   
            {isUserDislike  ?   (<i className='w-[24px] h-[24px] ' onClick={handleDissLike}> <ThumbsDownIcon />   </i>)
            :
            (<i className='w-[24px] h-[24px] text-red cursor-pointer' onClick={handleDissLike}> <ThumbsDownIcon />   </i> )
            }                    
